...
export function createLogger(serviceName: string, options: LoggerOptions = {}): winston.Logger {
  ...
  const structuredFormat = winston.format.combine(
    ...
    winston.format.json(),
    winston.format.printf(({ timestamp, level, message, service, component, requestId, userId, ...meta }) => {
      const logEntry: any = {
        '@timestamp': timestamp,
      ...
      logEntry.hostname = process.env.HOSTNAME || os.hostname();
  ...
  const consoleFormat = winston.format.combine(
    ...
    winston.format.printf(({ timestamp, level, message, service, component, requestId, ...meta }) => {
    ...
    try {
      ...
      transports.push(
        new LogstashTransport({
          port: logstashPort,
          node_name: `${serviceName}-${process.env.HOSTNAME || os.hostname()}`,
          host: logstashHost,
 ...
 */
export function createChildLogger(parentLogger: winston.Logger, context: LogContext = {}): winston.Logger {
 ...
 */
export function createHttpLoggerMiddleware(logger: winston.Logger) {
 ...
 */
export function createErrorLoggerMiddleware(logger: winston.Logger) {
  return (error: Error, req: ExtendedRequest, res: Response, next: NextFunction) => {
 ...
 */
export function setupGracefulShutdown(logger: winston.Logger): void {
